## Cleaning benthic data
## Description: removing sites, removing blank benthic minor categories, rename CHRYS to Rubble, and other labels
...
# # dont convert to factors until after summarise code...?
# # convert multiple columns to factors - use as.factor()
# # if just one... benthic$sitename <- as.factor(benthic$sitename)
# benthic <- 
#   benthic %>% 
...
# # site_transect is now a factor but needs to be converted to numeric to calculate SD and SE (convert to character first)
# # breast <- transform(breast, class=as.numeric(as.character(breast)))
# benthic <- transform(benthic, site_transect=as.numeric(as.character(benthic)))
# 
# str(benthic)
...
# calculate mean for each of the 6 transects for each site
  ...
  benthic_transects_mean <- select(benthic_transects_mean, -c(2:15))        # remove blank columns 
...
# mean benthic MAJOR only (including HALI)
  
  benthic_transects_mean_major <- select(benthic_transects_mean, 1:11, 64)      # select MAJOR benthic categories only
  ...
  # calculate site mean of the 6 transects #
    ...
    benthic_site_mean <- select(benthic_site_mean, -c(2:3))                
      ...
      benthic_site_sd <- select(benthic_site_se, -c(2:3))              
  ...
  # site mean benthic MAJOR only (including HALI)
  
  benthic_site_mean_major <- select(benthic_site_mean, 1:11, 64)      # select MAJOR benthic categories only
